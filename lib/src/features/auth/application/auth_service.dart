...
class AuthService extends _$AuthService {
  ...
  Future<void> loginToFlarumWithSocial(
    ...
    logger.info('开始Flarum社交登录流程，主机: $host, 类型: $type, UID: $socialUid');
    final api = FlarumApi();
    final originalBaseUrl = api.baseUrl;
    ...
    try {
      ...
      logger.debug('发送Flarum社交登录请求');
      final loginData = await api.loginWithJuhe(socialUid, type);
      logger.debug('收到Flarum社交登录响应: $loginData');

      final token = loginData['token'];
      final userId = loginData['userId'].toString();

      if (token == null) {
        throw Exception('Flarum社交登录响应缺少必要字段 (token)');
      }
      ...
      final profileData = await api.getUserProfile(userId);
      final attributes = profileData['data']?['attributes'] ?? {};
      final displayName = attributes['displayName'] ?? nickname;
      final username = attributes['username'] ?? nickname;
      final profileAvatarUrl = attributes['avatarUrl'] ?? avatarUrl;

      final accountId = '$userId@$host';
      logger.info('Flarum社交登录成功，用户ID: $userId, 账户ID: $accountId');
      ...
      final account = Account(
        ...
        name: displayName ?? username ?? 'Social User',
      ...
      logger.info('Flarum社交登录流程完成');
    } catch (e) {
      if (originalBaseUrl != null) {
        api.setBaseUrl(originalBaseUrl);
      }
      logger.error('Flarum社交登录失败: $e', e);
      throw Exception('Flarum 社交登录失败: $e');
    }
  }
