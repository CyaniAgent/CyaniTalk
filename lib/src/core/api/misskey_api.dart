...
class MisskeyApi extends BaseApi {
  ...
  Future<(List<dynamic>?, Exception?)> getNoteReactions(
    ...
    if (sinceId != null) data['sinceId'] = sinceId;
    if (untilId != null) data['untilId'] = untilId;
    if (sinceDate != null) data['sinceDate'] = sinceDate;
    if (untilDate != null) data['untilDate'] = untilDate;
    
    return executeApiCallSafe(
      'MisskeyApi.getNoteReactions',
      () => _dio.post('/api/notes/reactions', data: data),
      (response) => response.data as List<dynamic>,
      params: data,
      dioErrorParser: (error) => error.response?.data?['error']?['message'] ?? error.message,
    );
  }
}
