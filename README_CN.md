...
**Nyachi** 是一个基于 Flutter 开发的多元化跨平台社交客户端。它连接了 **Misskey**（去中心化的动态社交平台），旨在为用户提供从“广场喧嚣”到“深度讨论”的统一社区体验。

Nyachi 采用**响应式外壳 (Responsive Shell)** 设计，能够完美适配移动端（iOS/Android）和桌面端（Windows/macOS/Linux），并提供原生级的实时流体验与高级管理工具。
