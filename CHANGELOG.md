...
### 实时性与网络强化
- **网络层加固**：引入了全局 `RetryInterceptor`，针对超时、连接错误及特定的服务器错误（502/503/504）自动执行最多 **5 次** 指数退避重试，显著提升弱网环境下的实时性。
- **前台自动同步**：集成应用生命周期监听，当应用从后台恢复至前台时，自动触发 Misskey 串流重连并刷新通知列表。
- **强化重连机制**：优化了 `MisskeyStreamingService` 的重连逻辑，限制为最多 **5 次** 积极尝试（1s, 2s, 4s, 8s, 16s），并支持自动恢复断开前的所有频道订阅。
- **Flarum 动态监测**：在论坛页面引入实时端点校验，当绑定的 Endpoint 在设置中被移除时，页面将立即感知并回退至“未连接”状态。
- **智能预加载**：进入 Misskey 或 Flarum 页面及切换子标签时，自动触发数据有效性检测与静默刷新。

### 系统功能与修复
- **实时存储监测**：重构了存储设置页面，引入多层级堆叠进度条（Stacked Progress Bar），支持通过 OS 底层指令（PowerShell/df）实时获取盘符分区容量，并精确计算 App 全量物理占用。
- **存储 UI 优化**：引入分阶段渲染与“计算中...”加载占位，大幅提升进入存储设置页面的响应速度。
- **网络稳定性增强**：加固了 WebSocket 连接逻辑，增加了显式的就绪等待与连接超时保护，有效拦截 Windows 底层的信号灯超时错误。
- **辅助功能全量加固**：针对时间线、媒体查看器及主路由动画进行了深度优化，通过精准的 `ExcludeSemantics` 隔离与 `onComplete` 生命周期清理，彻底根除了 Windows 平台的 AXTree 更新错误。
